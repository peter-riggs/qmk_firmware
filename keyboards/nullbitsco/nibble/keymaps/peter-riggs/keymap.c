...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BASE] = LAYOUT_all(
    ...
    KC_GESC,   KC_LCMD, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT,  KC_PGUP,
    _______,   KC_LSFT, _______, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT, KC_UP,   KC_PGDN,
    MO(_VIA1), KC_LCTL, KC_LOPT, KC_LCMD,                            KC_SPC,                    _______, KC_RCMD, KC_ROPT, KC_LEFT, KC_DOWN, KC_RGHT
  ),

  [_VIA1] = LAYOUT_all(
              RESET,   KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  KC_DEL,  KC_SLEP,
    RGB_TOG,  _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______,  _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,          _______, _______,
    _______,  _______, _______, _______, _______, KC_CALC, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
...
const uint16_t CANVAS_HEIGHT     = 32;
const uint16_t CANVAS_WIDTH      = 128;
const uint16_t CANVAS_CHARS_SIZE = 512;

// 32 * 128 = 4096
// 4096 / 512 = 8
// one element is 8 pixels
char canvasBytes[512];
int  modulo = 2;

char bitsToByte(int8_t byteBits[]) {
    int byteValue       = 0;
    int highestBitValue = 128;
    int currentValue    = highestBitValue;
    for (int i = 7; i >= 0; i--) {
        if (byteBits[i]) {
            byteValue += currentValue;
        }
        currentValue /= 2;
    }
...
void applyEffectToCanvas(void) {
    int    currentByte = 0;
    int    bitCounter  = 0;
    int8_t byteBits[8];
    for (int y = 0; y < CANVAS_HEIGHT; y++) {
        for (int x = 0; x < CANVAS_WIDTH; x++) {
            byteBits[bitCounter] = (x * y) % modulo == 0;
            bitCounter++;
            if (bitCounter == 8) {
                canvasBytes[currentByte] = bitsToByte(byteBits);
                bitCounter               = 0;
                currentByte++;
            }
        }
...
void renderCanvas(void) {
    ...
    //         oled_write_pixel(x, y, (x * y) % modulo == 0);
    //         // oled_write_pixel(x, y, canvas[x * y]);
    //     }
    // }
    oled_write_raw(canvasBytes, sizeof(canvasBytes));
}
